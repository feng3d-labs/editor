...
export function useOAVVector3(props: OAVVector3Props)
{
    ...
    const vectorValue = computed(() => r_owner[props.name] as Vector3);
    const r_value = reactive(vectorValue.value || { x: 0, y: 0, z: 0 });

    // 同步值变化
    computed(() => {
        const val = vectorValue.value;
        if (val && (val.x !== r_value.x || val.y !== r_value.y || val.z !== r_value.z)) {
            r_value.x = val.x ?? 0;
            r_value.y = val.y ?? 0;
            r_value.z = val.z ?? 0;
        }
