...
### ✅ 阶段 6: MainView 和 Editor 迁移（第 12-13 步）【已完成】

#### ✅ 步骤 6.1: 迁移 MainView【已完成】
- [x] 创建 `src/vue-app/App.vue` - 主应用组件（已有占位版本）
- [x] 整合所有已迁移的视图（MainLayout 已包含所有视图）
- [x] 处理窗口大小调整（在 App.vue 中添加了 resize 监听）
- [x] 替换原有的 MainView，但保持功能一致（创建占位对象保持向后兼容）
- [ ] **验证**: 主界面布局正常，所有面板可以正常显示（待验证）

#### ✅ 步骤 6.2: 迁移 Editor 入口【已完成】
- [x] 修改 `src/Editor.ts`，移除 MainView 依赖
- [x] 保持 Egret 初始化（仍需要 popupLayer 等 Egret 组件）
- [x] 保持初始化流程不变
- [x] 创建占位对象保持向后兼容
- [ ] **验证**: 编辑器可以完整启动，所有功能正常（待验证）
